...
from datamodel.search.Ytan5_datamodel import Ytan5ProjectionLink
...
@Deleter(Ytan5ProjectionLink)
@Getter(Ytan5ProjectionLink)
class DeleteFrontierFrame(IApplication):
    ...
    def update(self):
        ...
        ls = self.frame.get(Ytan5ProjectionLink)
        print "Found ", len(ls), " links to delete."
        for l in ls:
            self.frame.delete(Ytan5ProjectionLink, l)
